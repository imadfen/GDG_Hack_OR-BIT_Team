...
import RateSubmission from '@/components/RateSubmission';
...
export default function page() {
    ...
    return (
        <div className='flex flex-col'>
            <Header />
            {judging ?
                <RateSubmission onCancel={() => setJudging(false)} onCreate={() => setJudging(false)} />
                :
                        ...
                        <div className="flex justify-center px-32 flex-grow">
                            <div className="w-full flex flex-col gap-5">
                                {Array(3).fill(null).map((_, i) => (
                                    <BluredCard key={i} className="w-full bg-white border rounded-3xl px-5 py-3 hover:scale-105 duration-200 cursor-pointer" onClick={() => setJudging(true)}>
